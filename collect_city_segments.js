...
const cityData = {
    ...
    "黑龙江省": ["哈尔滨市", "齐齐哈尔市", "牡丹江市", "佳木斯市", "大庆市", "鸡西市", "双鸭山市", "伊春市", "七台河市", "鹤岗市", "黑河市", "绥化市"],
    "上海市": ["上海市"],
    ...
    "新疆维吾尔自治区": ["乌鲁木齐市", "克拉玛依市", "吐鲁番市", "哈密市", "昌吉回族自治州", "博尔塔拉蒙古自治州", "巴音郭楞蒙古自治州", "阿克苏地区", "克孜勒苏柯尔克孜自治州", "喀什地区", "和田地区", "伊犁哈萨克自治州"]
};
...
function normalizeCity(province, city) {
    const cities = cityData[province];
    if (!cities) return city;
    
    for (const c of cities) {
        if (c.includes(city) || city.includes(c.replace('市', ''))) {
            return c;
        }
    }
    return city;
...
async function collectSegments() {
    ...
    for (const province of Object.keys(cityData)) {
        result[province] = {};
        const cities = cityData[province];
        ...
        for (const city of cities) {
            ...
            for (const [opName, prefixes] of Object.entries(operators)) {
                ...
                for (const prefix of prefixes) {
                    for (let i = 0; i < 30; i++) {
                        ...
                        if (location) {
                            ...
                            const normalizedCity = normalizeCity(normalizedProvince, location.city);
                            
                            if (normalizedProvince === province && normalizedCity === city) {
                                if (!result[province][city][opName].includes(segment)) {
                                    result[province][city][opName].push(segment);
                                }
                            }
                        }
                ...
                result[province][city][opName] = [...new Set(result[province][city][opName])].sort();
                console.log(`${province} ${city} ${opName}: ${result[province][city][opName].length} 个号段`);
            }
        }
    }
    
    console.log('\n数据已保存到 city_segments.json');
    fs.writeFileSync('city_segments.json', JSON.stringify(result, null, 4));
}
